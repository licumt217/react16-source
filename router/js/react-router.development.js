...
(function (global, factory) {
    ...
    function warning(cond, message) {
        if (!cond) {
            ...
            try {
                ...
                // enabling "pause on exceptions" in your JavaScript deb2ugger.
    ...
    function flattenRoutes(routes, branches, parentsMeta, parentPath) {
        ...
        routes.forEach((route, index) => {
            ...
            if (meta.relativePath.startsWith("/")) {
                !meta.relativePath.startsWith(parentPath) ? invariant(false, "Absolute route path \""
                    + meta.relativePath + "\" nested under path " + ("\"" + parentPath
                        + "\" is not valid. An absolute child route path ")
                    + "must start with the combined path of all its parent routes.") : void 0;
    ...
    function compilePath(path, caseSensitive, end) {
        ...
        warning(path === "*" || !path.endsWith("*") || path.endsWith("/*"), "Route path \""
            + path + "\" will be treated as if it were "
            + ("\"" + path.replace(/\*$/, "/*")
    ...
    function Route(_props) {
        invariant(false, "A <Route> is only ever to be used as the child of <Routes> element, "
    ...
    function Router(_ref3) {
        ...
        warning(location != null, "<Router basename=\"" + basename + "\"> is not able to match the URL "

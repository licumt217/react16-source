...
const axiosService_1 = __importDefault(require("../http/axiosService"));
const Util_1 = __importDefault(require("./Util"));
const StockUrls_1 = require("./StockUrls");
const StockParser_1 = __importDefault(require("./StockParser"));
//板块数据获取的开始时间
const plateBeginDate = '20230307';
const THE_START_DATE = '20230307';
const THE_END_DATE = '20501231';
//根据板块名称从Map中获取对应的数据获取URL
const getPlateUrlOfHistoryByName = (0, StockUrls_1.getPlateUrlOfHistoryFn)(plateBeginDate);
const getDayUrlOfHistoryByCode = (0, StockUrls_1.getDayUrlOfHistoryByCodeFn)(THE_START_DATE, THE_END_DATE);
exports.default = {
    /**
     * 根据给定大盘列表数据，判断最新的那天是否适合操作
     * @param entities
     */
    canOpOfPlate(entities) {
     ...
     * @param days
     */
    getLowestClosepriceInDays(entities, days) {
        let low = 9999999;
     ...
     * @returns
     */
    isSuperDown(stockList, days = 20, percent = -30) {
     ...
     * @returns
     */
    isHitStragety(stockList, strategy, downRatio = 0.7) {
        if (strategy === 'month') {
            ...
            const { close_price, name, code } = todayStock;
            return (name.toLowerCase().indexOf('st') === -1 &&
                name.toLowerCase().indexOf('退') === -1 &&
                ((max_close_price - close_price) / max_close_price) > downRatio);
        }
        else if (strategy === 'all') {
            const [todayStock, todayStock2, todayStock3, todayStock4, todayStock5, todayStock6, todayStock7,] = stockList;
            const { close_price, open_price, low, high, zf, cj_money, name, code } = todayStock;
            ...
            const shitiLengthPercent = ((close_price - open_price) / open_price) * 100;
            const shitiLengthPercent2 = ((close_price2 - open_price2) / open_price2) * 100;
            const shitiLengthPercent3 = ((close_price3 - open_price3) / open_price3) * 100;
            ...
            const firstLow = 0, lastLow = 0, theMax = 0;
            return (
     ...
     * @returns
     */
    getMaxClosePriceInDays(stocks, days, offset = 0) {
        let max = 0;
     ...
     * @returns
     */
    isStOrTuishi(stockName) {
        return stockName.includes('ST') || stockName.includes('退');
    },
    is300(code) {
        return (code.startsWith('300') || code.startsWith('688') || code.startsWith('301'));
    },
    /**
     ...
     * @returns
     */
    fetchLatestDayData(pageSize = 50) {
        return new Promise((resolve) => {
            const page = 1;
            const resultArray = [];
            const innerFetch = (page, pageSize, resultArray) => {
                const url = (0, StockUrls_1.getDayUrlOfLatestAll)(page, pageSize);
                Util_1.default.log(`开始获取stock数据..., page:${page}, pageSize:${pageSize}`);
                axiosService_1.default.get(url).then(({ data }) => {
                    const stockArray = StockParser_1.default.parseDatas(data);
                    if (!stockArray) {
                        const entities = StockParser_1.default.transStockArray2DetailDailyEntities(stockArray);
                        resolve(entities);
                    }
                    else {
    ...
    * @returns
    */
    fetchSingleHistoryDayData(code) {
        return new Promise((resolve) => {
            const url = getDayUrlOfHistoryByCode(code);
            axiosService_1.default.get(url).then(({ data }) => {
                const parsedData = StockParser_1.default.parseData(data);
                //如果stock code不对等情况，返回值会是null
                if (!parsedData) {
                    return resolve([]);
                }
                const { name, stockArray } = parsedData;
                const returnArray = StockParser_1.default.parseStockArray2DailyEntities(stockArray, code, name);
                resolve(returnArray);
            });
        });
    },
     ...
     * @returns
     */
    fetchMonthHistoryDataByCode(code) {
        return new Promise((resolve) => {
            const url = (0, StockUrls_1.getMonthHistoryUrl)(code);
            Util_1.default.log(`获取月线数据..., code:${code}`);
            axiosService_1.default.get(url).then((data) => {
                const stockArray = StockParser_1.default.parseDatas(data, "month");
                const entities = StockParser_1.default.transStockArray2DetailMonthEntities(stockArray);
                resolve(entities);
            });
        });
    },
     ...
     * @returns
     */
    fetchPlateHistoryDayDataByName(name) {
        return new Promise((resolve) => {
            const url = getPlateUrlOfHistoryByName(name);
            Util_1.default.log(`获取大盘数据name:${name}`);
            axiosService_1.default.get(url).then(({ data }) => {
                const parsedData = StockParser_1.default.parseData(data);
                //如果stock code不对等情况，返回值会是null
                if (!parsedData) {
                ...
                const returnArray = StockParser_1.default.parseStockArray2PlateEntities(stockArray, name);
                resolve(returnArray);
            });
        });
    },
     ...
     * @returns
     */
    fetchPlateLatestDayDataByName(plateName) {
        return new Promise((resolve) => {
            const url = (0, StockUrls_1.getPlateUrlOfLatestByName)(plateName);
            Util_1.default.log(`获取当天大盘数据name:${plateName}`);
            axiosService_1.default.get(url).then(({ data }) => {
                const parsedData = StockParser_1.default.parseData(data);
                //如果stock code不对等情况，返回值会是null
                if (!parsedData) {
                ...
                const { stockArray } = parsedData;
                const returnEntity = StockParser_1.default.parseStockArray2PlateEntities(stockArray, plateName);

...
import DetailMonthDAO from "../dao/detailMonthDAO";
...
const detailMonthDAO = new DetailMonthDAO();
class DetailMonthService {
    ...
    */
     ...
     * @returns
     */
    entities2Map(entities: DetailMonth[]): Map<string, DetailMonth> {
        const map = new Map();
     ...
     * @param entities
     */
    async batchAddSingleHistory(entities: DetailMonth[]): Promise<void> {
     ...
     * @param entities
     */
    async batchAddOneMonthDatas(entities: DetailMonth[]): Promise<void> {
        if (entities.length > 0) {
            await detailMonthDAO.batchAdd(entities)
        }
    }
     ...
     * @param entity
     */
    async add(entity: DetailMonth): Promise<void> {
     ...
     * @returns
     */
    transEntitiesDate2LocaleDate(entities: DetailMonth[]): DetailMonth[] {
        entities.forEach((entity) => {
    ...
    async getSingleFutureKLineDataArrayForMonth(
        date: string,
        codeArray: string[],
        months: number
    ): Promise<string[]> {
        ...
        for (let i = 0; i < codeArray.length; i++) {
            ...
            if (entities && entities.length > 1) {
                ...
                const zf: number =
                    ((end.close_price - start.close_price) / start.close_price) * 100;
    ...
    isGtMa5(entities: DetailMonth[], volume: string) {
        let all = 0;
        for (let i = 0; i < 5; i++) {
        ...
        if (volume) {
            return (
                (close_price > ma5 || close_price > ma10 || close_price > ma20) &&
     ...
     * @param entities
     */
    transEntities2EntityListByCode(
        entities: DetailMonth[],
        volume: string,
    ): DetailMonth[][] {
        const returnArray = [];
        let tempArray = [];
...
export default DetailMonthService;
...
(function (global, factory) {
    ...
    var View = {
        ...
        render: function render(_, ref) {// 生成虚拟dom VNode
            var props = ref.props;
    ...
    function normalizeLocation(
        ...
        var parsedPath = parsePath(next.path || '');//将path 解析为 {path,query,hash}
    ...
    var Link = {
        ...
        render: function render(h) {
            ...
            var handler = function (e) {
                if (guardEvent(e)) {//点击 router-link时，一些特殊情况不处理，直接返回。比如按下ctrl或右键等情况
                    if (this$1$1.replace) {
            ...
            if (scopedSlot) {
                if (!this.custom) {
                    !warnedCustomSlot && warn(false, 'In Vue Router 4, the v-slot API will by default wrap its content with an <a> element.'
                        + ' Use the custom prop to remove this warning:\n<router-link v-slot="{ navigate, href }" custom></router-link>\n');
                ...
                if (scopedSlot.length === 1) {
                ...
                } else if (scopedSlot.length > 1 || !scopedSlot.length) {
                    {
                        warn(
                            false,
                            ("<router-link> with to=\"" + (this.to) + "\" is trying to use a scoped slot but it didn't provide exactly one child."
                                + " Wrapping the content with a span element.")
                        );
                    }
...

            {
                ...
                if ('event' in this.$options.propsData && !warnedEventProp) {
                    warn(
                        false,
                        "<router-link>'s event prop is deprecated and has been removed in Vue Router 4."
                        + " Use the v-slot API to remove this warning: "
                        + "https://next.router.vuejs.org/guide/migration/#removal-of-event-and-tag-props-in-router-link."
                    );
    ...
    //- 点击 router-link时，一些特殊情况不处理，直接返回。比如按下ctrl或右键等情况
        ...
        Vue.mixin({
            beforeCreate: function beforeCreate() {
                if (isDef(this.$options.router)) {
                    ...
                    this._router = this.$options.router;//挂载到根实例上
    ...
    /**
     * 1、addRouteRecord。每个路由新建record
     * 2、路由的path和record添加到 pathList 和 pathMap
     * 3、返回pathList 和 pathMap
     * @param {*} routes 
     * @param {*} oldPathList 
     * @param {*} oldPathMap 
     * @param {*} oldNameMap 
    ...
    function createRouteMap(
        ...
        routes.forEach(function (route) {
            // 1、根据路由构建record。根据path构建正则表达式
            // 2、将path和对应的record添加到 pathList 和 pathMap
            addRouteRecord(pathList, pathMap, nameMap, route, parentRoute);
        });

        // ensure wildcard(未知的、通配符) routes are always at the end。
        // 一级路由* 通配符，位置挪到最后；
    ...
    /**
     * 1、根据路由构建record。根据path构建正则表达式
     * 2、将path和对应的record添加到 pathList 和 pathMap。一级路由和所有的下级子嵌套路由
     * @param {*} pathList 
     * @param {*} pathMap 
     * @param {*} nameMap 
     * @param {*} route 
    ...
    function addRouteRecord(
        ...
        var record = {
            path: normalizedPath,
            regex: compileRouteRegex(normalizedPath, pathToRegexpOptions),//根据路由path构建正则表达式
    ...
    /**
     * 1、根据 routes 构建 pathList 和 pathMap
     * 2、返回包含四个方法的对象：addRoute，addRoutes，getRoutes，match
     * @param {*} routes 
     * @param {*} router 
     * @returns 
     */
        ...
        function match(
            ...
            if (name) {//命名路由
                ...
                for (var i = 0; i < pathList.length; i++) {
                    ...
                    if (matchRoute(record$1.regex, location.path, location.params)) {//判断给定path是否和 路由表中路由的正则匹配
    ...
    /**
     * 判断给定path是否和 路由表中路由的正则匹配
     * 从实际url中解析出来路由参数，key是具体的key或pathMatch(*通配符)
     * @param {*} regex 
     * @param {*} path 
     * @param {*} params 
     * @returns 
    ...
    function flatMapComponents(
        matched,//Array<Record>
        fn
    ) {
        const mapResult = matched.map(function (m) {// m:Record
        ...
        return flatten(mapResult)
    }

    //- 数组扁平化 将 [[1],[2],[3]]转为 [1,2,3]
    ...
    History.prototype.confirmTransition = function confirmTransition(route, onComplete, onAbort) {
        ...
        //  * 把当前的路径队列和将要去往的路径队列进行对比长度，然后遍历最长的路径队列，从两个路径不一样的地方开始截取。
        //  * updated为之前一样的部分，
        //  * activated为将要去往路径的不一样的部分，
        //  * deactivated为当前路径的不一样的部分。
        ...
        var queue = [].concat(
            ...
            // global before hooks //调用 beforeEach
     ...
     * activated为将要去往路径的不一样的部分，
     * deactivated为当前路径的不一样的部分。
     * @param {*} current current.matched
     * @param {*} next route.matched
     * @returns 
     */
    function resolveQueue(
        ...
        return {
            updated: next.slice(0, i),//没有变化的部分
            activated: next.slice(i),//激活的路由组件
            deactivated: current.slice(i)//失活的路由组件
        }
    }

    //-
    function extractGuards(
        records,// deactivated 等。 Array<Record>
        name,  //守卫名字
        bind, // bindGuard
        reverse //是否反转
    ) {
    ...
    var HTML5History = (function (History) {
        ...
        HTML5History.prototype.setupListeners = function setupListeners() {
            ...
            // 1、点击浏览器的【前进】【后退】按钮，或者调用 history 对象的 back、forward、go 方法
    ...
    var HashHistory = (function (History) {
        ...
        HashHistory.prototype.setupListeners = function setupListeners() {
            ...
            // popstate 具体事件。点击前进或后退时，url的hash已经改变，拿着改变后的hash去调用 transitionTo()，触发路由组件更新。
            var handleRoutingEvent = function () {
                var current = this$1$1.current;
                if (!ensureSlash()) {
                    return
            ...
            window.addEventListener(//给window 绑定popstate事件
                eventType,
                handleRoutingEvent
            );
            this.listeners.push(function () {
    ...
    VueRouter.prototype.init = function init(app /* Vue component instance */) {
        ...
        // history注册回调函数，在路由改变时，遍历更新app的 _route值，因为是响应式的，触发路由组件更新
    ...
    //- 根据router-link 的 to属性匹配路由，新建href。

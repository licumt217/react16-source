...
// 格式化字符串时间
export function parseStrTime(str) {
  const reg = /(\d{2})(\d{2})(\d{2})/
  return str.replace(reg, '$1:$2:$3')
}

// 格式化字符串日期时间
export function parseStrDateTime(str, separate = '.') {
  if (!str) return
  let reg = null
  if (str.length === 8) {
    reg = /(\d{4})(\d{2})(\d{2})/
...
// 通过下载地址下载
export function downloadExcelByUrl(url) {
...
export function fillBlankContent(content) {
